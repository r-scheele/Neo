...
| Phase B status | Passed | B01-B05 are complete, B06 is next after approving its role/audit contract, and B07-B08 remain deferred in `docs/integration-prompts/backend-deferred/backend-deferred-index.md`. |
...
- Prepare B06 next after approving its role/audit contract for backend work; do not run B07-B08 until required auth/secrets/contracts are ready.
