...
| Phase B status | Passed | B01-B08 are complete for current backend commerce/permission/WhatsApp/AI draft workflows; later media/payment work remains deferred in `docs/integration-prompts/backend-deferred/backend-deferred-index.md`. |
...
| AI draft generation backend | Complete for MVP wiring; live QA and key rotation remain before production. |
...
- Do not run later provider/media/payment prompts until required auth/secrets/contracts are ready.
