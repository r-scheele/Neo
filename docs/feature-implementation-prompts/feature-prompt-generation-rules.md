...
- Every prompt must tell Codex to read `AGENTS.md` first.
...
- Every prompt must tell Codex to stop if required files are missing.
- Every prompt must include constraints, state rules, UI rules, validation, and stop conditions.
- Every prompt must tell Codex not to import design reference screenshots into app code.
- Every prompt must tell Codex to return files changed, what changed, how to test, risks, and a suggested commit message.
...
- Never ask Codex to build multiple backlog items in one prompt.
...
- If a prompt requires an app file, UI reference image, runtime asset, or package that is missing, Codex must stop and report the missing prerequisite.
- Do not ask Codex to install libraries inside feature prompts.
...
- TypeScript and lint pass, or Codex clearly reports why they could not run.
