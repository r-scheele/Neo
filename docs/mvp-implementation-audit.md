...
- Real WhatsApp sync, AI draft generation, receipt upload/OCR, payment-provider verification, and customer message memory remain deferred behind backend prompts.
- Core commerce records now have backend-backed paths for durable IDs, with isolated fixture/demo fallback for old local IDs.
...
| Create Order | `app/order/new.tsx`, `features/order/CreateOrderScreen.tsx`, `features/order/createOrderForm.ts` | Backend-backed for B06 | Controlled order form, validation, totals, backend submit state, analytics, server permission check, and audit write. | Old fixture/demo fallback remains isolated; signed-in B06 QA still needs to confirm audit rows. | Signed-in Clerk QA. | Public API base URL outside git. | Yes | Medium | B06 signed-in QA. | `enforce server permissions` |
| Order Detail | `app/order/[id].tsx`, `features/order/OrderDetailScreen.tsx`, `features/order/orderDetailData.ts` | Backend-backed for B06 | Detailed order view loads backend records by durable ID; cancel and delivery status changes use backend APIs for durable IDs. | Missing/local IDs can still show isolated demo fallback; reminders are not connected. | WhatsApp reminders, signed-in Clerk QA. | Public API base URL outside git. | Yes | Medium | B06 signed-in QA, later WhatsApp. | `enforce server permissions` |
...
| Receipt Review | `app/receipt/[id].tsx`, `features/receipts/ReceiptReviewScreen.tsx`, `features/receipts/receiptReviewData.ts` | Backend-backed for B06 | Trust-first UI loads backend receipt records, saves human review decisions, enforces owner/manager permission, and writes audit rows server-side. | No OCR, bank lookup, payment provider verification, customer message, or signed-in B06 audit QA yet. | Media/OCR/payment verification, signed-in Clerk QA. | Public API base URL outside git. | Yes | High | B06 signed-in QA, later media/OCR/payment work. | `enforce server permissions` |
| Follow-ups | `app/(tabs)/follow-ups.tsx`, `features/follow-ups/FollowUpsScreen.tsx`, `features/follow-ups/followUpQueueData.ts` | Backend-backed for B06 | Queue UI loads backend follow-ups; complete/reschedule persist to backend and write audit rows server-side. | No WhatsApp send or reminder engine; local fixture IDs remain isolated fallback. | WhatsApp send, optional notifications later, signed-in Clerk QA. | Public API base URL outside git. | Yes | Medium | B06 signed-in QA then B07. | `enforce server permissions` |
| Customer Profile | `app/customer/[id].tsx`, `features/customer/CustomerProfileScreen.tsx`, `features/customer/customerProfileData.ts` | Backend-backed for B05 | Profile UI loads safe backend metrics, order history, notes summary, and preferences. | Note editing and follow-up creation remain local/not connected. | Follow-up creation/update contract. | Public API base URL outside git. | Yes | Medium | Future customer/follow-up edit prompt. | `connect customer updates` |
