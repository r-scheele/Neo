...
| AI Approval Queue | `app/(tabs)/approvals.tsx`, `features/approvals/ApprovalQueueScreen.tsx`, `features/approvals/approvalQueueData.ts` | Mixed | Queue UI, search/filter, visual demo gates, states, analytics, and backend decision path for durable approval IDs. | Fixture approvals remain local; no AI draft generation or live context yet. | AI backend and live approval records. | Public API base URL outside git for durable approvals. | Yes | High | B08 AI workflow. | `connect approval queue workflow` |
