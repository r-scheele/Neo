...
| Today Command Center | `app/(tabs)/today.tsx`, `features/today/TodayCommandCenterScreen.tsx`, `features/today/todayCommandData.ts`, `stores/useOperationsStore.ts`, `stores/useConnectivityStore.ts` | Local-only | Polished Today screen, queue sections, loading/empty/error/offline/permission states, analytics, safe shared attention count, and shared last-synced UI metadata. | Queue cards and recommendations remain fixture/local; connectivity is local UI state, not real network truth. | Backend/API, real connectivity source. | None in client. | Yes | High | Backend/API boundary, loading/error hardening. | `connect today operations state` |
...
| Create Order | `app/order/new.tsx`, `features/order/CreateOrderScreen.tsx`, `features/order/createOrderForm.ts` | Local-only | Controlled order form, validation, totals, local submit state, analytics. | Saving generates `local-${Date.now()}` and routes to local mock detail. No order record. | Backend/API, operations store. | None in client. | Yes | High | Backend/API boundary. | `connect create order persistence` |
| Order Detail | `app/order/[id].tsx`, `features/order/OrderDetailScreen.tsx`, `features/order/orderDetailData.ts` | Local-only | Detailed order view, payment/delivery/timeline UI, states. | Orders are fixtures or generated local fallback; reminders/status/cancel are not connected. | Backend/API, permissions, audit logs. | None in client. | Yes | High | Backend/API boundary, permissions. | `connect order detail workflow` |
...
| Receipt Review | `app/receipt/[id].tsx`, `features/receipts/ReceiptReviewScreen.tsx`, `features/receipts/receiptReviewData.ts` | Local-only | Trust-first UI, receipt warning copy, decision modals, role gates, states, analytics. | Receipt details are fixtures; decisions are local; no OCR, bank lookup, payment provider, backend record, or customer message. | Backend/API, media/OCR/payment verification, server permissions, audit logs. | None in client. | Yes | High | Backend/API boundary, permissions. | `connect receipt review workflow` |
| Follow-ups | `app/(tabs)/follow-ups.tsx`, `features/follow-ups/FollowUpsScreen.tsx`, `features/follow-ups/followUpQueueData.ts` | Local-only | Queue UI, edit/send/mark done local states, offline/permission states, analytics. | No WhatsApp send, scheduling, backend record, or reminder engine. | Backend/API, WhatsApp send, optional notifications later. | None in client. | Yes | High | Backend/API boundary. | `connect follow up workflow` |
| Customer Profile | `app/customer/[id].tsx`, `features/customer/CustomerProfileScreen.tsx`, `features/customer/customerProfileData.ts` | Local-only | Profile UI, order/history sections, local note editing, states. | Customers, notes, history, preferences, and follow-up creation are fixtures/local. | Backend/API, permissions. | None in client. | Yes | High | Backend/API boundary. | `connect customer profile data` |
