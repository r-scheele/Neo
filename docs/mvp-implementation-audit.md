...
- Payment verification, OCR/media intake, WhatsApp/AI launch hardening, and remaining customer/team workflows still require backend integration. B05-B08 now provide durable commerce records, trusted permissions/audit coverage, backend WhatsApp workflow wiring, and server-side AI draft generation for current MVP endpoints.
...
| Inbox | `app/(tabs)/inbox.tsx`, `features/inbox/InboxConversationListScreen.tsx`, `features/inbox/inboxConversationData.ts` | Backend-backed for B07 | Search/filter UI, states, route links, analytics, and backend WhatsApp conversation list for durable records. | Old fixture conversations remain isolated fallback for local preview states. | Live WhatsApp QA and media handling. | Public API base URL outside git; Meta secrets in Supabase only. | Yes | Medium | Launch hardening. | `harden inbox workflow` |
| Conversation Detail / AI Draft | `app/conversation/[id].tsx`, `features/conversation/ConversationDetailScreen.tsx` | Backend-backed for B08 | Chat-style UI, backend WhatsApp thread loading, backend manual sends, server-generated AI drafts, approval routing, edit/takeover controls, and create order link. | Old fixture conversations and bundled demo drafts remain isolated fallback; attachments remain disabled. | Live WhatsApp/AI QA, media handling, and credential rotation. | Public API base URL outside git; OpenAI/Meta secrets in Supabase only. | Yes | Medium | Launch hardening. | `harden conversation workflow` |
| Create Order | `app/order/new.tsx`, `features/order/CreateOrderScreen.tsx`, `features/order/createOrderForm.ts` | Backend-backed for B06 | Controlled order form, validation, totals, backend submit state, server permission check, audit write, analytics. | Old fixture/demo fallback remains isolated; audit/mutation is not transaction-atomic yet. | Transaction-safe launch hardening. | Public API base URL outside git. | Yes | Medium | Launch hardening. | `enforce server permissions` |
| Order Detail | `app/order/[id].tsx`, `features/order/OrderDetailScreen.tsx`, `features/order/orderDetailData.ts` | Backend-backed for B06 | Detailed order view loads backend records by durable ID; cancel and delivery status actions call backend permission/audit endpoints. | Missing/local IDs can still show isolated demo fallback; reminders do not send WhatsApp. | WhatsApp send, transaction-safe launch hardening. | Public API base URL outside git. | Yes | Medium | B07 WhatsApp. | `connect whatsapp workflow` |
| AI Approval Queue | `app/(tabs)/approvals.tsx`, `features/approvals/ApprovalQueueScreen.tsx`, `features/approvals/approvalQueueData.ts` | Backend-backed for B08 | Queue UI, search/filter, states, analytics, backend approval list, AI draft approval details, and backend decision path for durable approval IDs. | Fixture approvals remain isolated fallback for local preview states. | Live approval QA and transaction-safe audit hardening. | Public API base URL outside git; OpenAI secret in Supabase only. | Yes | Medium | Launch hardening. | `harden approval workflow` |
| Receipt Review | `app/receipt/[id].tsx`, `features/receipts/ReceiptReviewScreen.tsx`, `features/receipts/receiptReviewData.ts` | Backend-backed for B06 | Trust-first UI loads backend receipt records and saves human review decisions through server permission/audit. | No OCR, bank lookup, payment provider verification, customer message, or staff workflow yet; audit/mutation is not transaction-atomic yet. | Media/OCR/payment verification, transaction-safe launch hardening. | Public API base URL outside git. | Yes | High | Later media/OCR/payment work. | `connect receipt media workflow` |
| Follow-ups | `app/(tabs)/follow-ups.tsx`, `features/follow-ups/FollowUpsScreen.tsx`, `features/follow-ups/followUpQueueData.ts` | Backend-backed for B06 | Queue UI loads backend follow-ups; complete/reschedule persist through server permission/audit. | No WhatsApp send or reminder engine; local fixture IDs remain isolated fallback. | WhatsApp send, optional notifications later. | Public API base URL outside git. | Yes | Medium | B07 WhatsApp. | `connect whatsapp workflow` |
...
- AI draft live QA, quality evaluation, and provider credential rotation.
...
- Transaction-atomic audit/mutation hardening for sensitive writes.
...
The app is ready for manual QA and remaining backend decision work. It is not ready for a real MVP launch because WhatsApp, AI, OCR/payment-provider workflows, transaction-safe audit hardening, and live Clerk/manual QA are not complete.
