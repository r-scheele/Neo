...
| --- | --- | --- | --- | --- |
| P0 | Remaining backend feature workflows are not implemented | B01-B07 are complete, but AI, OCR, and payment-provider verification are still deferred. | Continue through B08 and later media/payment work only when each prompt's prerequisites are ready. | Backend deferred B08 and later |
...
| P1 | Some operations state is still fixture-backed | Inbox, Conversation, and Follow-ups use backend records for durable WhatsApp IDs, but old local/demo IDs remain as isolated fallback and AI-generated approval context still depends on fixtures. | Keep fixture behavior honest and replace remaining AI workflows in B08. | Backend deferred B08 |
