...
-- Local migration only until remote push is explicitly approved.
-- RLS planning:
-- - Tables are tenant-scoped by business_id where applicable.
-- - Supabase Edge Functions use server-side authorization with Clerk claims.
-- - Public anon access policies are intentionally not added in this foundation pass.
-- - Add least-privilege RLS policies after server auth/profile bootstrap is reviewed.
